                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    float t_exit = std::min(std::min(t[dirIsNeg[0]].x, t[dirIsNeg[1]].y), t[dirIsNeg[2]].z);
    if (t_exit < 0)
        return false;
    float t_enter = std::max(std::max(t[1 - dirIsNeg[0]].x, t[1 - dirIsNeg[1]].y), t[1 - dirIsNeg[2]].z);
